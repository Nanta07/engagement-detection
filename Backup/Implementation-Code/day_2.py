...
def start_session(responden, sesi):
    ...
    with open(csv_file_path, mode='w', newline='') as file:
        ...
        writer.writerow(["Timestamp (Unix)", "Time (HH:MM:SS)", "Frame Name", "Engagement Level", "Response Time", "FPS"])
...
def capture_frames_from_webcam(session_folder):
    ...
    while cap.isOpened():
        ...
        with open(csv_file_path, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([timestamp, formatted_time, frame_name, engagement_level, response_time, fps])
